...
void bsDeployPlot::plotBS( int type )
{
  ...
  for (i=0; i<nPoints; i++) {
	...
	j=0;
	for (theta = 0; theta < 500; theta += 1) {
		x[j] = tmpX[0] + radio * cos((2.*M_PI/500.)*theta);
		y[j++] = tmpY[0] + radio * sin((2.*M_PI/500.)*theta);
	}
    pls->col0(opColor);
    pls->line(j, x, y);	
  }

// OPtypeID BStypeID posX posy
// 00 00  878.64  147.05
// 00 00  361.30  677.48
// 01 02  516.93  937.53
	...
	// tmpX[0] = 878.64;
	// tmpY[0] = 147.05;	
	// pls->poin(1,tmpX,tmpY,CIRCLE);
	
  	// pls->col0(GREEN);
	// pls->col0(YELLOW);
	// tmpX[0] = 361.30;
	// tmpY[0] = 677.48;	
	// pls->poin(1,tmpX,tmpY,TRIANGLE);
