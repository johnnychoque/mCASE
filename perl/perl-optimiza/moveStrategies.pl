...
sub moveResults
{
    ...
    $fileSrc_ = sprintf("results/%02d_%03d_datos_matlab.txt",$currentj_,$currenti_);
    ...
    $fileSrc_ = sprintf("results/%02d_%03d_bs_matlab.txt",$currentj_,$currenti_);
    ...
    $fileSrc_ = sprintf("results/%02d_%03d_users_matlab.txt",$currentj_,$currenti_);
    ...
    for($k_ = 1 ; $k_ <= 200; $k_++) {
    	...
    	$fileSrc_ = sprintf("results/%02d_%03d_final_solution_%04d.txt",$currentj_,$currenti_,$k_);
	...
	#$fileSrc_ = sprintf("%03d_qos_final_%04d.txt",$currenti_,$k_);
        $fileSrc_ = sprintf("results/%02d_%03d_qos_final_%04d.txt",$currentj_,$currenti_,$k_);
...
# The main!!!
# The first thing is to generate the variance values (input for the AR Model)

for($j = 1 ; $j <= $strategies_ ; $j++) {
